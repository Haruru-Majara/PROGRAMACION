...
public class MainJ {
	...
	public static void main(String[] args) {
		...
		HashSet<String> ficheros = new HashSet<>();
		...
		try {
			...
			for (Path path : Stream1) {
				...
				if (Files.isRegularFile(path)) {
					...
					/*
					 * le preguntamos si en la coleccion "ficheros" si no tiene los ficheros de esta
					 * carpeta que los agregue
					 */
					if (!ficheros.contains(path.getFileName().toString()))
						/*
						 * si no los tiene los guardamos en el coleccion los path to string porque el
						 * hashset es de String "ficheros"
						 */
			...
			for (Path path : Stream2) {
				...
				if (Files.isRegularFile(path)) {
					...
					/*
					 * le preguntamos si en la coleccion no tiene los ficheros de esta carpeta que
					 * los agregue
					 */
					if (!ficheros.contains(path.getFileName().toString()))
						/* si no los tiene los guardamos en el coleccion los paths "ficheros" */
						ficheros.add(path.getFileName().toString());
				}
			}

			/* una separacion bonita para mostrar los datos */
			System.out.println("***************************************************************");
			System.out.println("*********************** Datos Coleccion ***********************");
			System.out.println("***************************************************************");
